...
##        local folder which has been set as the working directory:
...
## 3. GENERATE BACKGROUND POINTS WITHIN STUDY AREA
    ...
    ## COMBINE PRESENCE/BACKGROUND POINTS
      ...
      all.pts <- rbind(porc.pts, bg.pts)
        head(all.pts)
        table(all.pts$pres) # 10000 bg, 1293 pres
...
## 5. THIN POINTS
    ...
    ## thin bg points or just pres pts?
      head(bg.pts)
      dist.bg <- dist(bg.pts[,1:2])    
      summary(dist.bg)
  
      head(porc.pts)
      dist <- dist(porc.pts[,2:3]) 
      summary(dist) ## similar median distance for pres/bg points (~4) -- units are lat/lon?
      
    ## how to find best thinning distance? look at semivariograms? how do I make those?
       
    ## try 10 reps with thin parameter = 0.8 km (800 m)
      thinning <- thin(porc.pts, lat.col = 'y', long.col = 'x', spec.col = 'pres',
                       thin.par = 0.8, reps = 10, locs.thinned.list.return = TRUE,
                       write.files = TRUE, max.files = 5, out.dir = 'vlab_files/',
                       out.base = 'porc_thinned', write.log.file = TRUE, 
                       log.file = 'porc_thinned_full_log_file.txt', verbose = TRUE)

      plotThin(thinning) ## looks like ~2-6 repetitions are needed to retain max # records
      
    ## using thinning distance of 0.8 km reduced the number of points from 1293 to 1032
    ## maybe use a larger distance like 1 km?
      
      porcs.thin <- read.csv('vlab_files/porc_thinned_thin1.csv') 
    ...
    ## also background points (or do this at the same time, with porcS? think it ran out of memory)
...
## 4. EXTRACT RASTER VALUES
  ...
  ## simplify dataframe (pres, predictor values)
      ...
      write.csv(cur.data, 'vlab_files/cur.data.06288.csv')
